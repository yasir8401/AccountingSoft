...
namespace ErpOnClick.ErpMain.Areas.Admin.Controllers
{
    ...
    public class AccountLedgerController : Controller
    {
        ...
        public List<AccountLedgerReportModel> GenerateReportForAccountLedger(DateTime FromDate, DateTime ToDate, int AccountId)
        {
            ...
            List<VoucherLines> allgeneralEntriesList = _voucherLinesService.dbset().IncludeOptimized(c => c.Voucher).IncludeOptimized(c => c.Voucher.VoucherTypeNavigation).IncludeOptimized(x => x.Account).IncludeOptimized(c=>c.Account.ParentAccount).IncludeOptimized(c=>c.Account.ParentAccount.ParentAccount).ToList();
            ...
            foreach (var item in newgeneralEntriesList)
            {
                ...
                if (item.AccountId == AccountId)
                {
                    ...
                    if (item.Account.ParentAccount.ParentAccount.AccountNameEn == "Assests" || item.Account.ParentAccount.ParentAccount.AccountNameEn == "Expenses")
                    {
                        reportObject.Balance = item.Account.OpeningBalance + (reportObject.Debit - reportObject.Credit);
                    }
                    else if (item.Account.ParentAccount.ParentAccount.AccountNameEn == "Liabilities" || item.Account.ParentAccount.ParentAccount.AccountNameEn == "Revenue")
                    {
