...
namespace ErpOnClick.ErpMain.Areas.Admin.Controllers
{
    ...
    public class ChangesInEquityController : Controller
    {
        ...
        public ChangesInEquityViewModel GenerateReportForChangesInEquity()
        {
            ...
            ProfitLossController profitLossController = new ProfitLossController(_pllogger, _voucherLinesService, _accountsService, _CompanyService, _BranchService, _AccountTypesService, _AccountGroupsService, _applicationParameterService);
            ...
            changesInEquityViewModel.OCI_2021 = voucherLines.Where(c => (c.Account.ParentAccount.AccountNameEn == "Other comprehensive income for the period" || c.Account.ParentAccount.ParentAccount.AccountNameEn == "Other comprehensive income for the period") && c.CreatedDate.Value.Year == applicationParameters.FinancialYearEndDate).Sum(c => c.Credit);
            changesInEquityViewModel.OCI_2020 = voucherLines.Where(c => (c.Account.ParentAccount.AccountNameEn == "Other comprehensive income for the period" || c.Account.ParentAccount.ParentAccount.AccountNameEn == "Other comprehensive income for the period") && c.CreatedDate.Value.Year == applicationParameters.FinancialYearStartDate).Sum(c => c.Credit);
            ...
            changesInEquityViewModel.SP_2021 = voucherLines.Where(c => (c.Account.ParentAccount.AccountNameEn == "Share premium" || c.Account.ParentAccount.ParentAccount.AccountNameEn == "Share premium" ) && c.CreatedDate.Value.Year == applicationParameters.FinancialYearEndDate).Sum(c => c.Credit);
            changesInEquityViewModel.SP_2020 = voucherLines.Where(c => (c.Account.ParentAccount.AccountNameEn == "Share premium" || c.Account.ParentAccount.ParentAccount.AccountNameEn == "Share premium" ) && c.CreatedDate.Value.Year == applicationParameters.FinancialYearStartDate).Sum(c => c.Credit);

            changesInEquityViewModel.R_2021 = voucherLines.Where(c => (c.Account.ParentAccount.AccountNameEn == "Reserve" || c.Account.ParentAccount.ParentAccount.AccountNameEn == "Reserve" ) && c.CreatedDate.Value.Year == applicationParameters.FinancialYearEndDate).Sum(c => c.Credit);
            changesInEquityViewModel.R_2020 = voucherLines.Where(c => (c.Account.ParentAccount.AccountNameEn == "Reserve" || c.Account.ParentAccount.ParentAccount.AccountNameEn == "Reserve" ) && c.CreatedDate.Value.Year == applicationParameters.FinancialYearStartDate).Sum(c => c.Credit);
