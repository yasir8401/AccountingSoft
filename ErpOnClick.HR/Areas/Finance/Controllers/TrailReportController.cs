...
namespace ErpOnClick.ErpMain.Areas.Admin.Controllers
{
    ...
    public class TrailReportController : Controller
    {
        ...
        public List<TrailBalanceReportModel> GenerateReportForTrailBalance(string FromDate, string ToDate)
        {
            ...
            var allVouchersLines = _voucherLinesService.dbset().IncludeOptimized(x => x.Account.ParentAccount.ParentAccount.ParentAccount).IncludeOptimized(c => c.Voucher.VoucherLines).ToList();
            ...
            {
                if (lines.Select(c => c.Account.IsTransactionable == true).FirstOrDefault())
                {
                    ...
                    reportObject.Type = lines.Select(x => x.Account.ParentAccount.ParentAccount.AccountNameEn).FirstOrDefault();
                    reportObject.Head = lines.Select(x => x.Account.ParentAccount.AccountNameEn).FirstOrDefault();
                    reportObject.SubHead = lines.Select(x => x.Account.ParentAccount.AccountNameEn).FirstOrDefault();
            ...
            {
                if (account.IsTransactionable == true)
                {
                    TrailBalanceReportModel reportObject = new TrailBalanceReportModel();

                    reportObject.Code = account.AccountCode;
                    reportObject.Account = account.AccountNameEn;
                    reportObject.Type = account.ParentAccount.ParentAccount.AccountNameEn;
                    reportObject.Head = account.ParentAccount.ParentAccount.AccountNameEn;
                    reportObject.SubHead = account.ParentAccount.AccountNameEn;
                    listReportTrailBalance.Add(reportObject);
                }
            }
            #endregion
