...
namespace ErpOnClick.ErpMain.Areas.Admin.Controllers
{
    ...
    public class AccountsController : Controller
    {
        ...
        public AccountsController(IUserService UserService,ILogger<AccountsController> logger, ICompanyService CompanyService, IBranchService BranchService, IAccountsService AccountsService, IAccountTypesService AccountTypesService, IAccountGroupsService AccountGroupsService, ILookupsService lookupService)
        {
            _logger = logger;
        ...
        public IActionResult _list(int TypeId, int GroupId)
        {
            expObj.AccountIdList = _AccountsService.dbset()
                ...
                && (GroupId != 0 && c.AccountGroupId == GroupId || GroupId == 0) && c.Deleted == false)
                .ToList();
        ...
        public IActionResult edit(int id)
        {
            ...
            expObj.AccountNatureList = lookups.Where(x => x.LookupTypeId == LookupTypList.AccountNature_078).ToList();
        ...
        public ActionResult save(Accounts postedData)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                postedData.AccountTypeId = 15;
                postedData.Deleted = false;
                postedData.AccountTypeId = 15;
                ...
                else if (postedData.AccountCode.Length > 2 && postedData.AccountCode.Length <= 8)
                {
                    postedData.AccountLevel = 3;
                    //postedData.IsTransactionable = false;

                }
                else if (postedData.AccountCode.Length > 8 && postedData.AccountCode.Length <= 12)
                {
                    postedData.AccountLevel = 4;
                    //postedData.IsTransactionable = false;

                }
                else if (postedData.AccountCode.Length > 12 && postedData.AccountCode.Length <= 16)
                {
