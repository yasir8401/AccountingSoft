...
namespace ErpOnClick.EmpEducation.Areas.Admin.Controllers
{
    ...
    public class EmpEducationController : Controller
    {
        ...
        public async Task<IActionResult> edit(int empId, int id)
        {
            ...
            DAL.Models.EmpEducation empEducationDetails = _EmpEducationService.GetById(id);
            empEducationDetails = empEducationDetails == null ? new DAL.Models.EmpEducation() { EmpEducationId = null } : empEducationDetails;
            expObj.specialtyList = lookupList.Where(x => x.LookupTypeId == LookupTypList.Specialty_076).ToList();
            expObj.subSpecialtyList = lookupList.Where(x => x.LookupTypeId == LookupTypList.Subspecialty_077 && x.ParentCode == empEducationDetails.SpecialtyId).ToList();
            //if (empEducationDetails != null && empEducationDetails.SpecialtyId != null)
            //{

            //}
            //else
            //{ 
            //    expObj.subSpecialtyList = 
            //        }

            expObj.EmpEducationDetails = empEducationDetails;
            expObj.empId = empId;
            expObj.empDetails = _EmpService.GetById(empId);
        ...
        public async Task<ActionResult> update(EmpEducationInheritied postedData)
        {
            ...
            if (postedData != null)
            {
                if (postedData.EmpEducationId == null)
                {
                    await save(postedData);
                }
                else
                {
        ...
        [HttpPost]
        public ActionResult GetSubSpecialtyList(string id)
        {
            var lookupList = _LookupService.GetAll();
            var subSpeccialtyList = lookupList.Where(x => x.LookupTypeId == LookupTypList.Subspecialty_077 && x.ParentCode == id).ToList();

            return Json(new { result = subSpeccialtyList });
        }
